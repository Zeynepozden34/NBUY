...
namespace BlogApp.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        {
            var result = await _unitOfWork.Categories.GetAsync(a => a.Id == categoryUpdateDto.Id);
            if (result != null)
            {
        ...
        public async Task<IResult> Delete(int categoryId, string modifiedByName)
        {
            ...
            if (result)
            {
                var category= await _unitOfWork.Categories.GetAsync(a => a.Id == categoryId);
                category.IsDeleted = true;
                category.ModifiedByName = modifiedByName;
                category.ModifiedDate = DateTime.Now;
                await _unitOfWork.Categories.UpdateAsync(category).ContinueWith(t => _unitOfWork.SaveAsync());
                return new Result(ResultStatus.Success, $"{category.Name} başlıklı makale başarıyla silinmiştir.");
            }
        ...
        public async Task<IResult> HardDelete(int categoryId)
        {
            ...
            if (result)
            {
                ...
                return new Result(ResultStatus.Success, $"{category.Name} başlıklı makale başarıyla veritabanından silinmiştir.");
            }
            return new Result(ResultStatus.Error, "Böyle bir makale bulunamadı.");
        }
    }
