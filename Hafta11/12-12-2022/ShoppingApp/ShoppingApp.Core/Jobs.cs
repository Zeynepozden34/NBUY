...
namespace ShoppingApp.Core
{
    public static class Jobs
    {
        ...
        {            
            var extension=Path.GetExtension(image.FileName); //extensionla resmin uzantısını aldık uzantı .png gibi gelecek.
            
            var randomName =$"{Guid.NewGuid()}{extension}";
            
            var path = Path.Combine(Directory.GetCurrentDirectory(),"wwwroot/images",randomName); /*sitenin url'ini veriyor yani http://localhost:5002/wwwroot/images/jfdhfbjcsjlg.png */
            
            using (var stream=new FileStream(path, FileMode.Create)) //dosyayı yüklüyoruz usingi yazmamızın ndeni yüklndiği gibi silinsin
            {
                image.CopyTo(stream);
            }
            return randomName;
