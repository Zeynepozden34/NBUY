    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> Edit(UserAddDto userUpdateDto)
        {
            if (ModelState.IsValid)
            {
                ...
                var userRoles = await _userManager.GetRolesAsync(user);//userin hangi role ait oldupuna bulmak için yazdık.
                await _userManager.AddToRolesAsync(user, userUpdateDto.SelectedRoles.Except(userRoles).ToList<string>());  //şecili rolere bak dah önceden secili olanları çıkar. yeni eklenenleri tek güncelle.
                await _userManager.RemoveFromRolesAsync( 
                    user,
                    userRoles.Except(userUpdateDto.SelectedRoles).ToList<string>()
                    ); //userde daha önce seçili olmayan rollere bak, yeni rol çıkardığın zaman onu güncelle.
                TempData["Message"] = Jobs.CreateMessage("Başarılı", $"{user.UserName} kullanıcsı başarıyla güncellendi.","success");
                return RedirectToAction("Index", "User");
            }
        ...
        // Roller ve Kullanıcılar ile ilgili silme işlenlerini de tamamlayınız.
    }
}
