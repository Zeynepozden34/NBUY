...
namespace ShoppingApp.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(UserManager<User> userManager, SignInManager<User> signInManager, IEmailSender emailSender, ICardService cardManger)
        {
            _userManager = userManager;
            _signInManager = signInManager;
            _emailSender = emailSender;
            _cardManager = cardManger;
        }
        ...
        public async Task<IActionResult> Register(RegisterDto registerDto)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    // yeni kayıt olan kullanıcıya varsıyaln olarak user rolü giriyoeum.
                    await _userManager.AddToRoleAsync(user, "User");
                    // yeni kayıt olan kullanıcıya için alışveriş sepeti oluşturtorum.
                    await _cardManager.InitializeCard(user.Id);
                    TempData["Message"] = Jobs.CreateMessage("Bilgi", "Hesabınız Başarıyla oluşturulmuştur. Giriş yapbilirsiniz", "success");
