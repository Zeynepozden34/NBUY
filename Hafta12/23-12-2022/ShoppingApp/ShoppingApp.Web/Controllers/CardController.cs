    ...
    public class CardController : Controller
    {
        ...
        public async Task<IActionResult> Checkout(OrderDto orderDto)
        {
            if (ModelState.IsValid)
            {
                ...
                //2) Eğer kart numarası geçerliyse ödemeyi al. değiilse hata mesajı al.
                //3) Ödeme bşarılıysa
                ...
                Payment payment = PaymentProcess(orderDto); // aşağıda kodalrı yazdık burada çağırdık.
                if (payment.Status=="success")
                {
                    SaveOrder(orderDto,userId); // ödeme başarılı olunca veritabanına kaydet.
        ...
        {
            #region CardNumberComment
            /*
             * - CardNumber geçersiz karakterlerden(boşluk) temizlensin
             * - CardNumberin uzunluk kontrolünü yapalım
             * - sayısal değer kontrolü
             * -Luhn algoritmasını uygulamka için gerekenleri yap.
             */
            #endregion
            ...
            {
                if (!Char.IsNumber(chr)) // sıradaki karakter rakam değillse
                {
                    return false;
                }
            }
            #region LuhnAlgoritması
            int ovenTotal = 0; //çft
            int oddTotal = 0; //tek toplamı
            for (int i = 0; i < cardNumber.Length; i += 2)
            {
                ...
                addedOddNumber = addedOddNumber >= 10 ? addedOddNumber - 9 : addedOddNumber; // burada topalm 10 dan büyükse sayıdan 9 çıkar dedik.
        ...
        private Payment PaymentProcess(OrderDto orderDto)
        {
            ...
            //payment yaparken optiona ihtiyacımız var ondan oluşturdu. Oluştuğumuz hesaptan bilgiler alındı.
            Options options = new Options();
            options.ApiKey = "sandbox-jAujoCSbBwnDDzXRFNAUdRCsVXDoNqvX";
            options.SecretKey = "sandbox-YhOBUFSvpEcVpUeVpy006QeXcGP69uuK";
            options.BaseUrl = "https://sandbox-api.iyzipay.com";
            #endregion

            #region CreatePaymentRequist
            CreatePaymentRequest request = new CreatePaymentRequest
            {
                Locale = Locale.TR.ToString(),
                ConversationId = new Random().Next(1111111, 9999999).ToString(),
                Price = Convert.ToInt32(orderDto.CardDto.TotalPrice()).ToString(), //biz küsüratı virgülle gösterdik ama iyizco eşitlik ister bu nedenler inte cevirdik.
                PaidPrice = Convert.ToInt32(orderDto.CardDto.TotalPrice()).ToString(),
                Currency = Currency.TRY.ToString(),
                Installment = 1,//TAKSİT SAYISI
                BasketId = orderDto.CardDto.CardId.ToString(),
                PaymentChannel = PaymentChannel.WEB.ToString(), // Ödemenin nerede yapılacağı
                ...
                Buyer = new Buyer
                {
                    ...
                    RegistrationAddress = "Barbaros Bulvarı, Yıldız İş Hanı Merkezi Kat:3",
                    Ip = "85.34.112.78",
                    City = "Istanbul",
                    Country = "Turkey"
                },
                ShippingAddress = new Address
                {
                    ...
                    City = "Istanbul",
                    Country = "Turkey",
                    Description = "Barbaros Bulvarı, Yıldız İş Hanı Merkezi Kat:3"

                },
                BillingAddress = new Address
                {
                    ContactName = "Aylin Doğan",
                    City = "Istanbul",
                    Country = "Turkey",
                    Description = "Barbaros Bulvarı, Yıldız İş Hanı Merkezi Kat:3"
            ...
            foreach (var item in orderDto.CardDto.CardItems)
            {
                ...
                basketItem.Category1 = "Genel";// Bu bize aslında carddto içindeki productların kategori bilgilerini de doldurmamız gerektiğni hatırlattı.
