...
namespace ShoppingApp.Data.Concrete.EfCore.Repositories
{
    public class EfCoreProductRepository : EfCoreGenericRepository<Product>, IProductRepository
    {
        ...
        public async Task<List<Product>> GetSearchResultsAsync(string searchString, bool isApproved = true)
        {
            ...
            if (isApproved)
            {
                return await result.Where(p => p.IsApproved == isApproved && (p.Name.ToLower().Contains(searchString) || p.Description.ToLower().Contains(searchString)))
                .ToListAsync();
            }
            return await result.Where(p=>p.Name.ToLower().Contains(searchString) || p.Description.ToLower().Contains(searchString))
              .ToListAsync();



            // searcStringlerin küçük harfe çevirip Shoppcontecte productsların içinde onaylı olan ve yazılan isimdeki (küçük harfe çevirip) veya açıklama kısmının içerdip içermediğini arayıp liste halinde getirir.
        }
    }
}
